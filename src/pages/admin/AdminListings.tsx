...
export default function AdminListings() {
  ...
  const { data: listings, isLoading } = useAdminListings(filter === 'all' ? undefined : filter);
            ...
            return (
              <div key={l.id} className="flex flex-col gap-4 rounded-2xl border border-gray-100 bg-surface p-4 shadow-sm sm:flex-row sm:items-center">
                ...
                <div className="flex shrink-0 flex-wrap gap-2">
                  ...
                  {l.status !== 'publiee' && (
                    <Button size="sm" onClick={() => moderate(l.id, 'publiee')} loading={updateStatus.isPending}>
                      <CheckCircle2 className="h-4 w-4" /> Publier
                    </Button>
                  )}
                  {l.status !== 'suspendue' && (
                    <Button variant="danger" size="sm" onClick={() => moderate(l.id, 'suspendue')}>
